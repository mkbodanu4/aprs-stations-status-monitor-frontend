...
switch ($get) {
    case 'groups':
        $group_filter = trim(filter_var($_GET['group'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_THOUSAND));
        if ($group_filter) {
        ...
        $query = "SELECT * FROM `groups` " .
            (is_array($group_filter) && count($group_filter) > 0 ?
                " WHERE `group_id` IN (" . implode(",", $group_filter) . ")"
                : "") .
            ";";
        $groups_stmt = $db->prepare($query);
        $groups_stmt->execute();
    ...
    case 'status':
        ...
        LEFT JOIN `groups` `r` ON `r`.`group_id` = `c`.`group_id`" .
            (is_array($group_filter) && count($group_filter) > 0 ?
                " WHERE `c`.`group_id` IN (" . implode(",", $group_filter) . ")"
                : "") .
            ";";
